    ...
    while True:
        try:
            ...
            if e.body["__type"] == "com.amazonaws.dynamodb.v20120810#LimitExceededException":
                ...
                logging.info("An unexpected exception occurred: %s" % e)
                sys.exit(1)
    ...
    if tableQueue:
        while True:
            ...
            if not args.schemaOnly:
                ...
                if read_capacity is not None and int(read_capacity) != original_read_capacity:
...
def do_restore(dynamo, sleep_interval, source_table, destination_table, write_capacity):
    ...
    if table_global_secondary_indexes is not None:
        for gsi in table_global_secondary_indexes:
            ...
            if original_gsi_write_capacity < int(write_capacity):
                gsi["ProvisionedThroughput"]["WriteCapacityUnits"] = int(write_capacity)

                # Save data to update gsi throughput in case the schema is not being copied
                gsi_update_data.append({
                    "Update": {
    ...
    if not args.dataOnly:
        ...
        if len(gsi_update_data) > 0:
            dynamo.update_table(destination_table, global_secondary_index_updates=gsi_update_data)
            wait_for_active_table(dynamo, destination_table, "updated")
                ...
                while True:
                    try:
                        ...
                        if (e.body["__type"] ==
                            ...
                            logging.error("Unexpected exception while updating index: " + e.body["__type"])
                            sys.exit(1)
